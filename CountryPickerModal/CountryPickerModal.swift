...
public struct Country {
   ...
   public var flag: UIImage {
        return UIImage(named: "CountryPickerModal.bundle/Images/\(code.uppercased())",
            in: Bundle(for: CountryPickerModal.self), compatibleWith: nil)!
    }
    
    internal init(name: String, code: String, phoneCode: String, placeHolder: String) {
        self.name = name
        self.code = code
...
public class CountryPickerModal: NibView {
    ...
    weak public var dataSource: CountryPickerModalDataSource?
    weak public var delegate: CountryPickerModalDelegate?
    ...
    internal(set) public var selectedCountry: Country {
        get {
            return _selectedCountry
                ...
                ?? countries.first(where: { $0.code == "US" })!
        }
        set {
    ...
    public func showCountriesList(from viewController: UIViewController) {
        let countryVc = CountryPickerModalController(style: .grouped)
        countryVc.countryPickerModal = self
        if let viewController = viewController as? UINavigationController {
            delegate?.countryPickerModal(self, willShow: countryVc)
            viewController.pushViewController(countryVc, animated: true) {
                self.delegate?.countryPickerModal(self, didShow: countryVc)
            }
        } else {
    ...
    public var countries: [Country] = {
        ...
        let bundle = Bundle(for: CountryPickerModal.self)
        guard let jsonPath = bundle.path(forResource: "CountryPickerModal.bundle/Data/CountryCodes", ofType: "json"),
        ...
        if let jsonObjects = (try? JSONSerialization.jsonObject(with: jsonData, options: JSONSerialization
            ...
            for jsonObject in jsonObjects {
                ...
                guard let name = countryObj["name"] as? String,
                    ...
                    let placeHolder = countryObj["placeholder"] as? String,
                    let phoneCode = countryObj["dial_code"] as? String else {
                        continue
                }
                
                let country = Country(name: name, code: code, phoneCode: phoneCode, placeHolder: placeHolder)
...
extension CountryPickerModal {
    ...
    public func setCountryByPlaceHolder(_ placeHolder: String) {
        if let country = countries.first(where: { $0.placeHolder == placeHolder }) {
            selectedCountry = country
        }
    }
    ...
    public func getCountryByPlaceHolder(_ placeHolder: String) -> Country? {
        return countries.first(where: { $0.placeHolder == placeHolder })
    }
}


// MARK:- An internal implementation of the CountryPickerModalDelegate.
// Sets internal properties before calling external delegate.
extension CountryPickerModal {
    func didSelectCountry(_ country: Country) {
        selectedCountry = country
        delegate?.countryPickerModal(self, didSelectCountry: country)
    }
}
